...
후보 조회는 `documents(content_hash) WHERE is_deleted=0` 부분 인덱스로 현재 hash bucket만 읽는다.
stale global map이나 서로 다른 connection의 `data_version` 해석에 의존하지 않고,
target-absent writer 안에서 **같은 hash의 모든 live DB row**와 lexical source 존재를 두 번
확인한다. 외부 파일 삭제로 새 missing candidate가 생겼으면 transaction을 rollback하고 최신
집합으로 unique/ambiguous를 다시 판정한다. commit 직후에도 선택 source와 나머지 peer의 DB
tuple·source 존재를 다시 확인해 이후 변화는 다음 work item 또는 `rename_source_changed`로
남긴다.

이 선택의 일반 복잡도는 `O(F log D + ΣB)`(`B`는 target별 동일-hash bucket)이고, 대다수 문서의
hash가 유일한 일반 vault에서는 사실상 선형이다. 빈 문서나 동일 template가 대량 반복돼 모든
target이 큰 동일-hash bucket을 공유하면 최악 `O(K²)` peer stat가 될 수 있다. 전역 세대 map의
복잡한 invalidation보다 commit 경계의 최신 정합성을 우선하는 의도적 tradeoff이며, 부분 인덱스로
DB 전체 scan과 무관한 hash의 비용은 제거한다.
...
- recovery ID query는 설정한 page 크기를 넘지 않고 메모리는 문서 하나의 body와 pending ID
  page에 제한된다.
- 경쟁 없는 reindex 정상 경로는 work item당 본문을 한 번 읽고, 동일-hash source 결정은 live
  `content_hash` 부분 인덱스의 bucket만 검사한다. 일반 복잡도는
  `O(F log D + ΣB)`이고 동일 hash가 대량 반복되는 최악 `O(K²)` peer stat tradeoff를 수용한다.
