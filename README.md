...
- ⌨️ **키보드 워크플로** — **명령 팔레트**(Ctrl/⌘+P)·**퀵 스위처**(Ctrl/⌘+O, 없으면 새 문서 생성)·사이드바 토글(Ctrl/⌘+\\), 에디터 **`/` 슬래시 명령**(헤딩·목록·할 일·콜아웃·표·코드·날짜 등 삽입)
- ✍️ **에디터** — [EasyMDE](https://github.com/Ionaru/easy-markdown-editor)(CodeMirror) 기반 툴바·문법강조·전체화면. **미리보기는 서버 렌더러(`/api/preview`)에 연결**해 위키링크·콜아웃·체크박스·새니타이즈가 실제 보기와 정확히 일치(좌우 분할 미리보기 지원). `[[위키링크]]` 자동완성·`/` 슬래시 명령은 그대로, 이미지 드래그&드롭/붙여넣기 업로드, 실시간 단어/문자 수(CJK 글자당 집계). 라이브러리는 vendoring되어 **오프라인 동작**.
...
- **API 키 발급**: 리본의 ⚙️(설정)에서 발급(한 번만 표시). MCP 접속용입니다.
...
> 프런트엔드 서드파티 라이브러리는 모두 프로젝트에 포함(`src/llm_wiki/web/static/vendor/`)해 자체 서버(`/static/vendor/`)에서 제공하므로 **인터넷 없이 동작**합니다 — 그래프 시각화 Cytoscape.js(`cytoscape.min.js`), 마크다운 에디터 EasyMDE(`easymde.min.js`·`easymde.min.css`, FontAwesome 미사용·툴바 글리프는 CSS 제공). 버전 업데이트 시 해당 파일만 교체하세요.
