...
- **구조**: `db.py`(스키마/연결) · `services/`(auth·documents·users·audit) · `search.py` · `graph.py` · `indexing.py` · `mcp_server.py` · `web/`(FastAPI+Jinja2) · `metrics.py`·`ratelimit.py`·`logconf.py`·`config.py` · `_cli_impl.py`.
