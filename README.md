...
| `RRF_K` | 하이브리드 RRF 상수(클수록 순위 영향 평탄화) | `60` |
| `SEARCH_CANDIDATE_FACTOR` / `SEARCH_CANDIDATE_MIN` | BM25 오버페치 배수·하한 | `4` / `40` |
| `SEARCH_VECTOR_FACTOR` / `SEARCH_VECTOR_CAP` | 벡터 KNN 오버페치 배수·상한 | `3` / `600` |
| `SEARCH_TITLE_EXACT_BOOST` / `SEARCH_TITLE_PREFIX_BOOST` | 제목 정확/접두 일치 리랭크 부스트 | `0.05` / `0.015` |
| `SEARCH_PROXIMITY_WEIGHT` | 벡터 근접 리랭크(0=끔) | `0.0` |

#### 검색 튜닝 프리셋 (복붙용)

기본값은 일반 지식베이스에 맞춰져 있습니다. 코퍼스 성격에 따라 `.env`에만 바꿔 넣으면 됩니다(재배포·재인덱싱 불필요, 서버 재시작만).

| 프리셋 | 추천 값 | 언제 |
|--------|---------|------|
| **기술문서** (API·스펙, 제목 일치 중요) | `SEARCH_TITLE_EXACT_BOOST=0.08`, `SEARCH_TITLE_PREFIX_BOOST=0.03`, 나머지 기본 | 제목·경로 단서가 강한 엔지니어링 vault |
| **일기·노트** (의미 탐색 위주) | `SEARCH_VECTOR_FACTOR=4`, `SEARCH_PROXIMITY_WEIGHT=0.02`, `SEARCH_TITLE_EXACT_BOOST=0.02` | 제목이 약하고 본문 의미가 중요한 경우 |
| **혼합(기본)** | 표의 기본값 그대로 | 대부분 vault |

한국어·CJK: BM25만으로는 공백 없는 합성어 내부 부분일치가 약할 수 있습니다. **하이브리드(기본 모드)** 를 유지하고, BM25만 쓸 때는 검색 화면의 하이브리드 권장 안내를 따르세요.
...
로그인 후 화면은 옵시디언풍 **앱 셸**로 구성됩니다 — 맨 왼쪽 **리본**(아이콘 도구막대), 그 옆 **좌측 사이드바**(파일 트리 / 검색 / 태그 탭), 가운데 **본문**, 오른쪽 **패널**(목차·탐색(백링크/나가는 링크/의미 유사)·활동), 맨 아래 **상태바**(단어/문자/백링크 수). 좌·우 사이드바는 접거나 경계선을 끌어 폭을 조절할 수 있고 상태는 브라우저에 저장됩니다. `editor`/`admin`은 새 문서 작성·편집·삭제가 가능합니다.
