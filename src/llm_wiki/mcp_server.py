...
def create_mcp_server(app: AppContext) -> FastMCP:
    ...
    @mcp.tool(description="Hybrid search (BM25 + embedding vector, RRF-fused). 'count' is the "
                          ...
                          "overview from a long reference without reading it), 'section_depth' "
                          "(nesting depth of the matched heading, 1=top-level; null if none), and "
                          "'char_start'/'char_end' + 'context_preview' (the matched chunk's exact "
                          "body offset range and a few plain-text lines around the match — judge "
                          "relevance or deep-link without a read_chunk round-trip; null for the "
                          "rare hit with no indexed chunk). "
